...
void encoder_open() {
  ...
  //水回り
  pinMode(PIN_ENC_A_W, INPUT);
  pinMode(PIN_ENC_B_W, INPUT);
  digitalWrite(PIN_ENC_A_W, HIGH);
  digitalWrite(PIN_ENC_B_W, HIGH);
  attachInterrupt(PIN_ENC_A_W, enc_change_w, CHANGE);
}
//水回り
void W_encoder_reset(){
  count_w = 0;
}

static void enc_change_w() {
  int32_t a_curr = digitalRead(PIN_ENC_A_W);
  int32_t b_curr = digitalRead(PIN_ENC_B_W);
